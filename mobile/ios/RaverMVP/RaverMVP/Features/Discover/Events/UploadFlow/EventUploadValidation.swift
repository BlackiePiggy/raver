...
enum EventUploadValidation {
    static func issues(for draft: EventUploadDraft) -> [EventUploadValidationIssue] {
        ...
        if case .create = draft.mode, draft.selectedTimeZoneLookup == nil {
            issues.append(.init(step: .time, message: LT("请搜索城市并确认活动时区。", "Search a city and confirm the event timezone.", "都市を検索してイベントのタイムゾーンを確認してください。")))
        }
        for (index, slot) in draft.lineupSlots.enumerated() {
            ...
            if names.count != slot.actType.performerCount || names.contains(where: { $0.isEmpty }) {
                issues.append(.init(step: .lineup, message: LT("第 \(index + 1) 个阵容条目有未填写的 DJ，请补全或删除。", "Lineup item #\(index + 1) has empty DJ names. Complete or remove it.", "\(index + 1)番目のラインナップに未入力のDJがあります。入力または削除してください。")))
            }
            if !draft.stageEntries.isEmpty,
               slot.stageName.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                issues.append(.init(step: .lineup, message: LT("第 \(index + 1) 个阵容条目请选择舞台。", "Choose a stage for lineup item #\(index + 1).", "\(index + 1)番目のラインナップでステージを選択してください。")))
            }
            if (slot.startTime == nil) != (slot.endTime == nil) {
                issues.append(.init(step: .lineup, message: LT("第 \(index + 1) 个阵容条目的开始和结束时间需要同时填写。", "Start and end time for lineup item #\(index + 1) must be filled together.", "\(index + 1)番目の開始時間と終了時間は両方入力してください。")))
            }
        }
