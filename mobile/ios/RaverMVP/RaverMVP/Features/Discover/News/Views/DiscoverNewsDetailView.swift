...
struct DiscoverNewsDetailView: View {
    ...
    private func sendSharePayload(
        ...
        _ = try await appContainer.shareMessageRepository.sendNewsCardMessage(
    ...
    @MainActor
    private func openNewsQRCode() async {
        do {
    ...
    @MainActor
    private func openNewsPoster() async {
        do {
            let resolved = try await shareLinkCoordinator.resolveLink(
                target: ShareTarget(
                    type: .news,
    ...
    @MainActor
    private func saveNewsPoster() async {
        do {
            let resolved = try await shareLinkCoordinator.resolveLink(
                target: ShareTarget(
                    type: .news,
