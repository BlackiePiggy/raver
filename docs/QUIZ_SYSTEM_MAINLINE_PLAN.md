...
- 若题目包含媒体资源，则必须在正式进入答题前完成整场出题预加载与坏题替换后才开始计时
- 题库由 web 端后台维护
...
- [x] 当前只做中文
- [x] 当前抽题规则为全局随机抽取
- [x] 增加 admin-only 的 `debug 套题`，用于调试不同题目排版
- [x] 未来会预留标签 / 难度 / 题型等扩展口
- [x] 每次答题固定抽取 `20` 题
- [x] 通过规则为“答对数量达到 N”
- [x] `N` 可由 web 后台调整
...
- [x] 用户允许主动重新开始整场答题
- [x] 单题超时后自动判错并直接跳下一题
- [x] 点击开始答题后先进入“正在出题”页，整场题目媒体预加载完成后再正式开始
- [x] 正式随机场次若某题媒体连续加载失败，则在开考前用候补题快速替换
- [x] 若开考前最终仍无法凑齐完整题量，则本次不进入答题流程，前置失败不应白白消耗次数
...
- [x] 普通身份用户只能看到正式“开始答题”
- [x] admin 身份用户在 iOS 开始页额外看到“进入调试套题”
- [x] `debug 套题` 由 web 后台从已有题库中选题组成
- [x] `debug 套题` 不消耗正式每日次数
- [x] `debug 套题` 不影响通过资格或认证结果
...
- [x] `allowRestartDuringSession`
  - V1 为 `true`
- [x] `debugQuestionIds`
  - admin-only 调试套题题目 id 列表，按保存顺序出题
- [x] `updatedAt`
...
1. 进入答题入口
2. 查看开始页
3. 普通用户点击“开始答题”后向服务端创建随机 session
4. admin 用户可额外点击“进入调试套题”创建 debug session
5. 一次性拿到本次题目的展示 payload
6. 进入“正在出题”加载页，客户端对整场正式题做媒体预加载
7. 若某正式题媒体失败，则在开考前用候补题替换并锁定本场最终题单
8. 全部最终题单资源就绪后进入第 1 题并开始倒计时
9. 作答 / 超时 / 跳题后进入下一题
10. 全部完成后提交
11. 结果页只显示：
   - 答对 `X/20`
...
- [x] admin-only 显示
- [x] 由 web 后台从现有题库中选择题目组成，顺序可控
- [x] iOS 开始页仅 admin 额外显示“进入调试套题”按钮
- [x] debug session 不消耗正式每日次数
- [x] debug session 不写入通过资格
- [x] debug session 复用同一套题目页、预加载、计时、结果页 UI
...
- [x] 整场只有在“最终实际出题列表”的媒体全部准备好后，才允许进入第 1 题并开始计时
- [x] 计时开始点以后，不能因为图片慢加载而损耗用户作答时间

### V1 方案

- [x] 服务端一次性返回正式题列表与候补题列表
- [x] iOS 客户端在答题开始后先进入“正在出题”页
- [x] iOS 在正式开考前完成整场最终题单媒体预加载
- [x] 当前题资源包括：
  - [x] 题干图片
  - [x] 各选项图片
...
- [x] 正式随机场次若某题仍失败，则在开考前尝试替换为候补题
- [x] 若候补题也不足以补齐完整题量，则本次不进入答题
- [x] 该失败过程不进入倒计时
- [x] 结果页不额外暴露失败原因，因为失败不会带着残缺题单进入答题

### 性能原则

- [x] 当前主线接受“开始前先把最终 20 题全部媒体准备好再开考”
- [x] 通过候补题替换降低因坏图导致的开考失败概率
- [x] 避免首屏等待过长
...
- [x] 新增独立 tab
- [x] 支持从已有题库中选择题目加入调试套题
- [x] 支持移除调试套题题目
- [x] 支持展示当前调试套题顺序
- [x] 保存后写回服务端 `debugQuestionIds`
...
- [x] 建立最小服务端状态结构
- [x] 确认不保存长期逐题答案明细
- [x] 为 `QuizConfig` 增加 `debugQuestionIds`
...
- [x] 实现每日次数限制
- [x] 实现永久通过状态写入
- [x] 增加 `debug_set` session 创建模式
- [x] `debug_set` 仅 admin 可用
- [x] `debug_set` 不消耗次数、不写入资格
- [x] `quiz/status` 返回 admin 可见的 debug 套题可用状态
- [x] create session 返回正式题 + 候补题
- [x] submit session 按最终实际展示题单计分
- [x] 开考前失败回滚次数
...
- [x] 新建答题中断 / 放弃 / 重启交互
- [x] admin-only 调试套题入口
- [x] iOS 开始页区分正式开始 / 调试套题开始
- [x] iOS 调试套题启动走独立 session mode
...
- [x] 已新增 `quiz:preflight:smoke` 脚本，覆盖候补题替换 / preflight 回滚次数 / 按最终展示题单计分三条主线
- [x] 当前 smoke 在目标数据库上会先检查 quiz 新迁移是否已落库；若缺少 `debug_question_ids` 等新字段，会明确提示先迁移再执行
...
- [x] iOS Quiz 放弃 / 重启确认弹窗已改为显式传入 alert 类型，避免 `Alert(item:)` 关闭时清空绑定导致确认按钮偶发无效
- [x] Web quiz 新建 / 编辑题目表单已去掉手填排序值，`correctOptionId` 改为只读联动，且新题默认继承全局默认时长
- [x] Server quiz 题目 `sortOrder` 已改为后端自动维护：创建时未传则自动追加到末尾，更新时未传则保留原值
