...
- [x] 待完成：使用真实腾讯云 `SDKAppID` / `UserSig` 做真机或模拟器联调。记录时间：待完成
...
- [x] 待确认：语音消息是否需要最长时长上限提示。记录时间：待完成
...
- [x] 待执行：拿真实腾讯云测试账号验证会话列表、1v1 文本、群文本。计划时间：待定
- [x] 待执行：选一个真实 Squad 执行 `/v1/im/tencent/squads/:squadId/sync`，确认腾讯云 IM 会话列表出现群会话。计划时间：待定
- [x] 待执行：补齐语音发送、消息实时接收、滚动刷新与未读联动。计划时间：待定
- [x] 待执行：收口私聊入口全链路（用户主页发起私聊 -> C2C 会话创建/拉起 -> 会话列表可见 -> 聊天页首屏可发）。计划时间：待定
- [x] 待执行：收口会话列表排序与未读同步细节，对齐腾讯 IM Demo 行为。计划时间：待定
- [x] 待执行：按腾讯云 UIKit 口径补齐会话页与聊天页操作逻辑对齐清单（回复、@、已读、长按菜单）。计划时间：待定
- [x] 待执行：继续做第二批视觉对齐（会话列表 cell 信息层级、长按菜单样式与动作排序）。计划时间：待定
- [x] 待执行：列出 OpenIM 删除清单。计划时间：待定
