...
- [x] 活动 / 主办方 / DJ 工作区首页已继续收口到产品态样式，统一按钮、卡片和页面分区语言
- [x] 活动 / 主办方 / DJ 三类核心目录页已继续收口到新的产品态样式，统一筛选条、摘要卡、状态区与分页操作
...
- [x] Event 主链路已继续补齐到目录管理 + 活动新建 + 活动编辑的后台原生可用闭环
- [x] Event Studio 已补封面 / 阵容图上传、替换、移除与提交清空语义，活动编辑素材管理可直接使用
- [x] Event 新建 / 编辑页已切到统一后台产品态操作区与结果跳转，活动管理链路已可从目录一跳进入并继续回到编辑
- [x] Event 目录中心已补更多筛选、当前页多选、批量复制与单条一跳编辑能力，活动目录管理效率已继续提升
- [x] Event 目录中心已前置批量指定主办方与批量清空主办方能力，不必先跳活动绑定中心再做高频批量治理
- [x] Event 目录中心单页摘要已收口为 10 条，进一步控制目录层首屏和分页请求压力
- [x] Benchmark DJ 200 与 Benchmark Event 相关测试数据已从数据库清理，避免污染正式后台目录
