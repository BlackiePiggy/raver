  ...
  return (
    <>
      <motion.nav
        initial={{ y: -100 }}
        animate={{ y: 0 }}
        className={`fixed w-full transition-all duration-300 ${isOpen ? 'z-[120]' : 'z-50'} ${
          scrolled || isOpen ? 'bg-neutral-950/80 backdrop-blur-md py-4 border-b border-white/5' : 'py-8 bg-transparent'
        }`}
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[100] flex flex-col items-center justify-center gap-8 bg-neutral-950 md:hidden"
          >
            {navItems.map((item) => (
              <Link
                key={item.name}
