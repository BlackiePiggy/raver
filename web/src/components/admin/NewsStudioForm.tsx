...
export default function NewsStudioForm({
  ...
  const orderedResourceUrls = useMemo(() => {
    const coverUrl = draft.coverImageUrl.trim();
    if (!coverUrl) return resourceUrls;
    const withoutCover = resourceUrls.filter((url) => url !== coverUrl);
    return resourceUrls.includes(coverUrl) ? [coverUrl, ...withoutCover] : resourceUrls;
  }, [draft.coverImageUrl, resourceUrls]);

  const previewAssets = useMemo<OverlayImageViewerAsset[]>(
    () =>
      orderedResourceUrls.map((url, index) => ({
        url,
        alt: `news-resource-${index + 1}`,
        title: draft.coverImageUrl.trim() === url ? '封面素材' : `资源 ${index + 1}`,
        subtitle: draft.coverImageUrl.trim() === url ? '当前封面' : '正文资源',
      })),
    [draft.coverImageUrl, orderedResourceUrls]
  );
  ...
  const handleResourceRailWheel = (event: React.WheelEvent<HTMLDivElement>) => {
  ...
  const copyResourceLink = async (url: string) => {
    ...
    try {
      ...
      showToast(error instanceof Error ? error.message : '复制资源链接失败。', 'error');
    }
  };
  ...
  const insertResourceIntoBody = (url: string) => {
    ...
    updateBody(nextBody);
    showToast('已插入正文，并保持编辑焦点。', 'success');
  };
  ...
  const removeResource = async (url: string) => {
    ...
    if (needsDraftCleanup) {
      try {
        ...
        showToast(
          error instanceof Error ? error.message : '草稿资源清理失败，已取消本次移除。',
          'error'
        );
    ...
    setCleaningResourceUrl(null);
    showToast(
      needsDraftCleanup ? '已移除资源，并完成草稿资源清理。' : '已移除资源。',
      'success'
    );
  };
  ...
  const handleUploadFiles = async (fileList: FileList | null) => {
    ...
    try {
      ...
      if (!successCount) {
        showToast('上传成功但没有返回可用的资源地址。', 'error');
        return;
      }

      showToast(`资源上传完成：${successCount}/${files.length} 张。`, 'success');
    } catch (error) {
  ...
  return (
    <div className="space-y-5">
      ...
      <Section
        title="核心内容"
        description="先完成标题、来源、分类和发布时间，确保资讯主体信息完整。"
      >
      ...
      <Section
        title="摘要与正文"
        description="先完成摘要、原始链接和封面信息，再进入正文工作区。Markdown 原文与渲染结果并排独占，并限制高度为固定容器内滚动。"
      >
        <div className="space-y-5">
                ...
                <div className="mt-4 overflow-hidden rounded-[24px] border border-[#e8eceb] bg-[#f8f9f8]">
                  {draft.coverImageUrl.trim() ? (
                    <button
                      type="button"
                      onClick={() => {
                        const coverIndex = previewAssets.findIndex((asset) => asset.url === draft.coverImageUrl.trim());
                        setPreviewResourceIndex(coverIndex >= 0 ? coverIndex : 0);
                      }}
              ...
              <div className="admin-reference-card p-4">
                ...
                {orderedResourceUrls.length ? (
                  <div className="relative mt-4">
                    ...
                    <div
                      ref={resourceRailRef}
                      onWheel={handleResourceRailWheel}
                      className="flex gap-3 overflow-x-auto px-2 pb-2"
                    >
                      {orderedResourceUrls.map((url, index) => {
                        ...
                        return (
                          <div
                            key={url}
                            className={`w-[132px] shrink-0 space-y-2 rounded-[20px] border bg-white p-3 transition ${
                              isCover
                                ? 'border-[#071110] shadow-[0_0_0_2px_rgba(7,17,16,0.12),0_12px_24px_rgba(7,17,16,0.12)]'
                                : 'border-[#e8eceb]'
                            }`}
                          >
                            <button
                              type="button"
                              onClick={() => setPreviewResourceIndex(index)}
                              className={`block h-[96px] w-[96px] overflow-hidden rounded-[18px] border bg-[#f8f9f8] ${
                                isCover ? 'border-[#071110] shadow-[0_0_0_2px_rgba(7,17,16,0.08)]' : 'border-[#edf0f2]'
                              }`}
                            >
                              {/* eslint-disable-next-line @next/next/no-img-element */}
                            ...
                            <div className="flex min-h-[20px] flex-wrap gap-1">
                              {isCover ? (
                                <span className="rounded-full bg-[#071110] px-2 py-0.5 text-[10px] font-semibold text-white">
                                  封面
                                </span>
                              ) : null}
                              {isCleaning ? (
                                <span className="rounded-full bg-[#f4f5f7] px-2 py-0.5 text-[10px] font-semibold text-[#6b7280]">
                                  清理中
                                </span>
                              ) : null}
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              <button
                                type="button"
                                onClick={() => void copyResourceLink(url)}
                                className="inline-flex h-9 items-center justify-center rounded-full border border-[#d7ded9] bg-white text-[#18211f]"
                                aria-label="复制资源链接"
                                title="复制链接"
                              >
                            ...
                            <button
                              ...
                              disabled={isCleaning}
                              className="inline-flex h-9 w-full items-center justify-center rounded-full border border-[#ead6d6] bg-white text-[#8b3a3a] disabled:opacity-60"
                              aria-label="移除资源"
                              title="移除资源"
                            >
      ...
      <Section
        title="关联对象"
        description="使用共享绑定能力添加多个 DJ、品牌和活动，操作逻辑与新的后台绑定层保持一致。"
      >
      ...
      <Section
        title="提交"
        description="确认正文和关联对象后即可保存，出站 payload 仍保持当前后端契约兼容。"
      >
      ...
      <OverlayImageViewer
        ...
        activeIndex={previewResourceIndex}
        onClose={() => setPreviewResourceIndex(null)}
        onChange={setPreviewResourceIndex}
      />
    </div>
  );
}
