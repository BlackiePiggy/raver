...
export default function AccountDeletionsAdminPage() {
  ...
  if (isLoading) {
    return (
      <AdminAppShell title="账号删除请求" description="加载账号删除请求与媒体清理状态中。">
  ...
  if (!user || !canOperate) {
    return (
      <AdminAppShell title="账号删除请求" description="当前账号暂时没有账号删除管理权限。">
  ...
  return (
    <AdminAppShell
      title="账号删除请求"
      eyebrow="Raver Admin / Privacy Ops"
      description="查看账号删除请求、IM 删除与 OSS 媒体清理状态，并在需要时执行重试或到期处理。"
    >
