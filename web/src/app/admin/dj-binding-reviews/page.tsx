...
export default function DJBindingReviewsAdminPage() {
  ...
  const selectedCandidates = useMemo(() => {
    ...
    return (selectedJob?.candidates ?? []).filter((candidate) => selected.has(candidate.id));
  }, [selectedCandidateIds, selectedJob]);
    ...
    <main className="min-h-screen bg-bg-primary text-text-primary">
      ...
      <section className="mx-auto max-w-7xl space-y-5 px-6 pb-12 pt-24">
        ...
        <div className="grid gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
          <section className="rounded-lg border border-border-secondary bg-bg-secondary p-5">
            ...
            <div className="mt-4 space-y-3">
              ...
              {!jobs.length && !listLoading && (
                <div className="rounded-lg border border-dashed border-border-secondary px-4 py-8 text-center text-sm text-text-secondary">
                  暂无 DJ 绑定审核任务
                </div>
              )}
            </div>
          ...
          <section className="rounded-lg border border-border-secondary bg-bg-secondary p-5">
            {!selectedJob ? (
              <div className="rounded-lg border border-dashed border-border-secondary px-4 py-12 text-center text-sm text-text-secondary">
                请选择左侧一个 DJ 绑定审核任务
              </div>
            ) : (
              <div className="space-y-5">
                ...
                <div className="flex items-center justify-between gap-3 rounded-lg border border-border-secondary bg-bg-tertiary px-4 py-3">
                  <div>
                ...
                <div className="space-y-3">
                  {(selectedJob.candidates ?? []).map((candidate) => {
                    ...
                    return (
                      <div key={candidate.id} className="rounded-lg border border-border-secondary bg-bg-tertiary p-4">
