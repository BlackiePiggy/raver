...
const normalizeSubmissionLineupArtists = (
  ...
  if (!source) {
    return normalizeCanonicalLineupArtists([], fallbackSlots);
  }
...
const createOrUpdateEventFromSubmission = async (payload: Prisma.JsonObject, submitterId: string) => {
  const name = cleanText(payload.name);
...
const createEntityFromSubmission = async (entityType: string, payload: Prisma.JsonObject, submitterId: string) => {
  switch (entityType) {
    case 'event':
      return createOrUpdateEventFromSubmission(payload, submitterId);
    case 'dj':
